...
resource "aws_lambda_function" "sdwan_phase5" {
  provider         = aws.virginia
  function_name    = "sdwan-phase5"
  description      = "SD-WAN Phase 5 - Advanced routing demo: dummy interfaces, BGP communities, route-maps"
  role             = aws_iam_role.sdwan_lambda_execution_role.arn
  handler          = "phase5_handler.handler"
  runtime          = "python3.12"
  timeout          = 600
  memory_size      = 256
  filename         = data.archive_file.lambda_package.output_path
