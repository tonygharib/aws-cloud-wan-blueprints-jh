...
# Fix VyOS config directory permissions — the LXD container creates these
# as root:root but VyOS needs root:vyattacfg for the vyos user to commit.
lxc exec router -- chgrp -R vyattacfg /opt/vyatta/config/active/
lxc exec router -- chmod -R g+rw /opt/vyatta/config/active/
lxc exec router -- chgrp -R vyattacfg /opt/vyatta/etc/quagga/
lxc exec router -- chmod -R g+rw /opt/vyatta/etc/quagga/
